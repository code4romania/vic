...
export const Stepper = ({ steps, currentStep, completedSteps, goToStep }: StepperProps) => {
  return (
    <div className="bg-white border-1 order-cool-gray-30 shadow-lg flex flex-row rounded py-3 px-6 justify-around items-center">
      {steps.map((step, index) => {
        ...
        return (
          ...
          >
            ...
            <p
              className={`hidden md:block text-sm text-cool-gray-500 ${isActive && 'text-yellow'} ${isCompleted && 'text-cool-gray-900 hover:text-yellow'}`}
            >
              {step.label}
            </p>
          </div>
