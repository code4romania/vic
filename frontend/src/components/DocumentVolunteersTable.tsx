		...
		cell: (row: IVolunteer & { completionStatus: VolunteerDataCheck }) => (
			<div className='flex gap-2 w-full items-center'>
				...
				<div className=''>
					...
					{row.completionStatus?.isIncomplete && <ExclamationCircleIcon
						width={20}
						height={20}
						color="red"
						data-tooltip-id={`error-tooltip-${row.user.id}`}
					/>}
					{row.completionStatus?.isIncomplete && <Tooltip
...
const DocumentVolunteersTable = ({ query, setQuery, selectedVolunteers, setSelectedVolunteers }: DocumentVolunteersProps) => {
	...
	const onSort = (column: TableColumn<IVolunteer & { completionStatus: VolunteerDataCheck }>, direction: SortOrder) => {
		setQuery(
			{
	...
	return (
		<PageLayout>
			...
			<Card>
				<CardHeader><h2>{i18n.t('side_menu:options.volunteers_list')}</h2></CardHeader>
				<CardBody>
					{query?.volunteerStatus === VolunteerStatus.ACTIVE && (
						<DataTableComponent
							...
							pagination
							selectableRowDisabled={(row: IVolunteer & { completionStatus: VolunteerDataCheck }) => row.completionStatus.isIncomplete}
							selectableRows
