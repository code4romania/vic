...
import { isIdentityDataIncomplete } from '../common/utils/document-contracts.helpers';
import { IdentityDataIncompleteModal } from '../components/IdentityDataIncompleteModal';
...
const Home = ({ navigation }: any) => {
  ...
  // display incomplete identity data warning modal if there's any identity data missing
  const isUserIdentityDataIncomplete = useMemo(() => {
    return userProfile ? isIdentityDataIncomplete(userProfile) : false;
  }, [userProfile]);
  useEffect(() => {
    if (isUserIdentityDataIncomplete) {
      bottomSheetRef.current && setTimeout(() => bottomSheetRef.current?.present(), 500);
    }
  }, [isUserIdentityDataIncomplete]);
  ...
  return (
    <>
      ...
      <IdentityDataIncompleteModal bottomSheetRef={bottomSheetRef} navigation={navigation} />
    </>
  );
};
